...
# Features
Hash table implementation with adjustable size.
Support for inserting key-value pairs into the hash table.
Searching for a specific key and retrieving its associated value.
Deleting a key and its associated value from the hash table.
Graceful handling of invalid user inputs.
# Usage
Install Python if not already installed.
Clone or download the project files.
Open a terminal or command prompt and navigate to the project directory.
Run the script by executing the following command: python interactive_hash_table.py
Enter the size of the hash table as prompted.
# Choose from the available options:
--------------------------------------------------------|
Option 1: Insert a key-value pair.                      |
